...
boardWIDTH		= 12				# to be set by user input later...
boardHEIGHT		= 5					# ...
...
def display_window( p ):
	...
	for y in range(boardHEIGHT):
		for x in range(boardWIDTH):
			...
			n = board[x][y]
			tilecolour = c64colour[n+1]
			if n>0:
				pygame.draw.rect(window, tilecolour, [x1,y1,tileWIDTH,tileHEIGHT])
				pygame.draw.rect(window, pygame.Color('white'), [x1,y1,tileWIDTH,tileHEIGHT],2)
			else:
				pygame.draw.rect(window, pygame.Color('gray'), [x1+1,y1+1,tileWIDTH-2,tileHEIGHT-2],1)
	# Header text
	txtInfo = basicFont.render('PENTOMINOS', ANTI_ALIASED, pygame.Color('white'), pygame.Color('black'))
	...
	txtInfoPos = txtInfo.get_rect()
	txtInfoPos.midbottom = window.get_rect().midbottom
	window.blit(txtInfo, txtInfoPos)
